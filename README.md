...
- an installed Codex CLI or Claude Code CLI
...
Before consuming a pairing, the bridge confirms the selected provider CLI
version, Node.js and macOS versions, and the exact Keychain write path. Run
`surfaces-bridge diagnose --provider codex` (or `claude`) to repeat those local
checks without contacting Surfaces or consuming a pairing.
